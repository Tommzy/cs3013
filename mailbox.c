...
asmlinkage long RcvMsg(pid_t *sender, void *msg, int *len, bool block){
	...
	if(usr_msg != NULL)	copy_to_user(&msg->text, usr_msg, msg->length);
	if(sender_pid != NULL)	copy_to_user(&msg->sender_pid, sender_pid, sizeof(pid_t));
	if(usr_msg_len != NULL)	copy_to_user(&msg->length, usr_msg_len, sizeof(int));
...
asmlinkage long RcvMsg(pid_t * dest, void *msg, int len, bool, block) {
	CS3013_message *msg;
	int downReturn;
	struct task_struct *self = current;
